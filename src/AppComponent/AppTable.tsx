...
import { Eye, Edit, Trash2, MoreHorizontal, Inbox } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export interface GlobalTableProps<RowType extends { _id: string }> {
  ...
  emptyMessage?: string;
  loading?: boolean;
  maxHeight?: string; // optional: to enable scroll
}
...
export function AppTable<RowType extends { _id: string }>({
  ...
  loading = false,
  maxHeight = "400px", // default scroll height
              ...
              : data.map((row) => (
                  ...
                  >
                    {columns.map((col) => (
                      ...
                      >
                        {col.render ? col.render(row) : <span className="truncate">{String(row[col.key] ?? "")}</span>}
                      </TableCell>
                    ))}
